...
# https://networkx.readthedocs.io/en/latest/_modules/networkx/algorithms/cuts.html
...
import networkx_boundary as nx_boundary
...
def cut_size(G, S, T=None, weight=None):
    ...
    *cut size* is the sum of the weights of the edges "between" the two
    ...
    """
    edges = nx_boundary.edge_boundary(G, S, T, data=weight, default=1)
    if G.is_directed():
        edges = chain(edges, nx_boundary.edge_boundary(G, T, S, data=weight, default=1))
...
def volume(G, S, weight=None):
    ...
    degree_S = degree(S, weight=weight)
    return sum(d for d in degree_S.itervalues())
